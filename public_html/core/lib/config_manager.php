...
class AConfigManager
{
    ...
    public function getFormFields($group, $form, $data)
    {
        ...
        /** @see _build_form_appearance() */
        /** @see _build_form_mail() */
        /** @see _build_form_im() */
        /** @see _build_form_api() */
     ...
     * @throws AException
     */
    public function validate(string $group, ?array $data = [], ?int $store_id = 0)
    {
        if (!$group || !$data) {
            return false;
        }
        ...
        foreach ($data as $dataName => $dataValue) {
            switch ($group) {
                case 'details':
                    $config_url = $config_ssl_url = '';
                    if ($dataName == 'store_name' && !$dataValue) {
                        $this->errors['name'] = $this->language->get('error_name');
                    }
                    if ($dataName == 'config_title' && !$dataValue) {
                        $this->errors['title'] = $this->language->get('error_title');
                    }
                    //URLS validation
                    if ($dataName == 'config_url' && $dataValue) {
                        $config_url = $dataValue;
                    }
                    if ($dataName == 'config_ssl_url' && $dataValue) {
                        $config_ssl_url = $dataValue;
                    }
                    if ($dataName == 'config_url'
                        && (!$dataValue
                            || !preg_match("/^(http|https):\/\//", $dataValue))
                    ) {
                    ...
                    elseif ($dataName == 'config_ssl_url' && $dataValue && !has_value($data['config_url'])) {
                        $saved_settings = $this->mdl->getSetting($group, $store_id);
                        $config_url = $saved_settings['config_url'];
                        $config_ssl_url = $dataValue;
                    } //when quicksave only url
                    elseif ($dataName == 'config_url' && $dataValue && !has_value($data['config_ssl_url'])) {
                        $saved_settings = $this->mdl->getSetting($group, $store_id);
                    ...
                    if (isset($data['config_owner'])
                        && (
                            mb_strlen($data['config_owner']) < 2
                            || mb_strlen($data['config_owner']) > 64
                        )
                    ) {
                        $this->errors['owner'] = $this->language->get('error_owner');
                    }

                    if (isset($data['config_address'])
                        && (
                            mb_strlen($data['config_address']) < 2
                            || mb_strlen($data['config_address']) > 256
                        )
                    ) {
                        $this->errors['address'] = $this->language->get('error_address');
                    }

                    if (isset($data['store_main_email'])
                        && (
                            mb_strlen($data['store_main_email']) > 96
                            || (!preg_match(EMAIL_REGEX_PATTERN, $data['store_main_email']))
                        )
                    ) {
                        $this->errors['email'] = $this->language->get('error_email');
                    }

                    if (isset($data['config_telephone']) && mb_strlen($data['config_telephone']) > 32) {
                        $this->errors['telephone'] = $this->language->get('error_telephone');
                    }
                    if (isset($data['config_postcode']) && mb_strlen($data['config_postcode']) < 3) {
                        $this->errors['postcode'] = $this->language->get('error_postcode');
                    }
                    if (isset($data['config_city']) && mb_strlen($data['config_city']) < 2) {
                ...
                case 'appearance':
                    ...
                    foreach ($item_name as $key) {
                        foreach (['width', 'height'] as $dim) {
                            if ($dataName == 'config_image_' . $key . '_' . $dim && !$dataValue) {
                ...
                case 'checkout':
                    if ($dataName == 'config_start_order_id' && $dataValue && !(int)$dataValue) {
                        $this->errors['start_order_id'] = $this->language->get('error_start_order_id');
                    }
                    if ($dataName == 'starting_invoice_id' && $dataValue && !(int)$dataValue) {
                        $this->errors['starting_invoice_id'] = $this->language->get('error_starting_invoice_id');
                    }
                    if ($dataName == 'config_expire_order_days' && $dataValue && !(int)$dataValue) {
                ...
                case 'mail':
                    if (($data['config_mail_protocol'] == 'smtp')
                        && (($dataName == 'config_smtp_host' && !$dataValue)
                            || ($dataName == 'config_smtp_port' && !$dataValue)
                            || ($dataName == 'config_smtp_timeout' && !$dataValue))
                    ) {
                        $this->errors['mail'] = $this->language->get('error_mail');
                    }

                    if ($dataName == 'config_alert_emails'
                        && $dataValue
                        && !preg_match(self::ALERT_EMAILS_REGEX, $dataValue)
                    ) {
                ...
                case 'system':
                    if ($dataName == 'config_error_filename' && !$dataValue) {
                        $this->errors['error_filename'] = $this->language->get('error_error_filename');
                    }
                    if ($dataName == 'config_upload_max_size') {
                        $data[$dataValue] = preformatInteger($dataValue);
                    }
                    break;
                default:
                    break;
        ...
        return ['error' => $this->errors, 'validated' => $data];
    }
    ...
    protected function _build_form_details($form, $data)
    {
        ...
        $fields['ssl_url'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
                'name'     => 'config_ssl_url',
        ...
        $fields['title'] = $form->getFieldHtml(
            $props[] = [
                'type'         => 'input',
        ...
        $fields['meta_description'] = $form->getFieldHtml(
            $props[] = [
                'type'         => 'textarea',
        ...
        $fields['meta_keywords'] = $form->getFieldHtml(
            $props[] = [
                'type'         => 'textarea',
        ...
        $fields['description'] = $form->getFieldHtml(
            $props[] = [
                'type'         => 'texteditor',
        ...
        $fields['owner'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
                'name'     => 'config_owner',
        ...
        $fields['postcode'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
        ...
        $fields['address'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'textarea',
                'name'     => 'config_address',
        ...
        $fields['country'] = $form->getFieldHtml(
            $props[] = [
                'type'            => 'zones',
        ...
        $fields['latitude'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['longitude'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['email'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
        ...
        $fields['telephone'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['language'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['admin_language'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
                'name'    => 'admin_language',
        ...
        $fields['auto_translate_status'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['translate_src_lang_code'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['translate_method'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['translate_override_existing'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['warn_lang_text_missing'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['currency'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['currency_auto'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['length_class'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['weight_class'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['tax_class'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
    ...
    protected function _build_form_general($form, $data)
    {
        ...
        $fields['product_default_sort_order'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['stock_display'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['nostock_autodisable'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['stock_status'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['display_reviews'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['reviews'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
                'name'    => 'enable_reviews',
        ...
        $fields['download'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['help_links'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['show_tree_data'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['embed_status'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['embed_click_action'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['account_create_captcha'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['account_recaptcha_v3'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['recaptcha_site_key'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['recaptcha_secret_key'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
    ...
    protected function _build_form_checkout($form, $data)
    {
        ...
        $fields['tax_store'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['tax_customer'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['start_order_id'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['invoice'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['invoice_prefix'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
                'name'  => 'invoice_prefix',
        ...
        $fields['customer_group'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['customer_price'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['customer_approval'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['customer_email_activation'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['guest_checkout'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['checkout'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['stock_checkout'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['total_order_maximum'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['total_order_minimum'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['order_status'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['customer_cancelation_order_status'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'multiselectbox',
                'name'    => 'config_customer_cancelation_order_status_id[]',
                'value'   => $data['config_customer_cancelation_order_status_id'] ?? [],
                'options' => $order_statuses,
                'style'   => 'chosen',
            ]
        );

        $fields['expire_order_days'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['cart_weight'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['shipping_session'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['cart_ajax'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['zero_customer_balance'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['shipping_tax_estimate'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['coupon_on_cart_page'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
    ...
    protected function _build_form_appearance($form, $data)
    {
        ...
        if (!$data['tmpl_id']) {
            ...
            $fields['template'] = $form->getFieldHtml(
                $props[] = [
                    'type'    => 'selectbox',
            ...
            $fields['admin_template'] = $form->getFieldHtml(
                $props[] = [
                    'type'    => 'selectbox',
                    'name'    => 'admin_template',
            ...
            $fields['admin_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
                    'name'     => 'admin_width',
            ...
            $fields['image_grid_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_grid_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_product_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_product_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['storefront_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_popup_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_popup_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_category_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_category_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_manufacturer_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_manufacturer_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_additional_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_additional_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_related_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_related_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_cart_width'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
            ...
            $fields['image_cart_height'] = $form->getFieldHtml(
                $props[] = [
                    'type'     => 'input',
    ...
    protected function _build_form_mail($form, $data)
    {
        ...
        $fields['smtp_username'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['smtp_password'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'password',
        ...
        $fields['smtp_port'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
        ...
        $fields['smtp_timeout'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
        ...
        $fields['alert_emails'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'textarea',
    ...
    protected function _build_form_im($form, $data)
    {
        ...
        foreach ($protocols as $protocol) {
            ...
            $fields[$protocol]['driver'] = $form->getFieldHtml(
                $props[] = [
                    'type'    => 'selectbox',
            ...
            if (!$no_driver) {
                $fields[$protocol]['storefront_status'] = $form->getFieldHtml(
                    $props[] = [
                        'type'       => 'checkbox',
                ...
                $fields[$protocol]['admin_status'] = $form->getFieldHtml(
                    $props[] = [
                        'type'       => 'checkbox',
    ...
    protected function _build_form_api($form, $data)
    {
        $fields = [];
        //api section
        $fields['storefront_api_status'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['storefront_api_key'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['storefront_api_stock_check'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['admin_api_status'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['admin_api_key'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['admin_access_ip_list'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['task_api_key'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
                'name'  => 'task_api_key',
    ...
    protected function _build_form_system($form, $data)
    {
        ...
        $fields['voicecontrol'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        if (!defined('ENCRYPTION_KEY')) {
            $fields['encryption'] = $form->getFieldHtml(
                $props[] = [
                    'type'  => 'input',
                    'name'  => 'encryption_key',
        ...
        $fields['seo_url'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
                'name'  => 'enable_seo_url',
        ...
        $fields['retina_enable'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['image_quality'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['compression'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'input',
        ...
        $fields['cache_enable'] = $form->getFieldHtml(
                $props[] = [
                    'type'  => 'checkbox',
        ...
        $fields['error_log'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['debug'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
                'name'    => 'config_debug',
        ...
        $fields['debug_level'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
        ...
        $fields['template_debug'] = $form->getFieldHtml(
            $props[] = [
                'type'  => 'checkbox',
        ...
        $fields['error_filename'] = $form->getFieldHtml(
            $props[] = [
                'type'     => 'input',
        ...
        $fields['system_check'] = $form->getFieldHtml(
            $props[] = [
                'type'    => 'selectbox',
